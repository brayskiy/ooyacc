...
void Reader::reader(void)
{
    char buf[2 * 1024];

    sprintf(buf, banner, file_prefix);
    fprintf(code_file, "%s", buf);
    Symtab::create_symbol_table();
    read_declarations();
    read_grammar();
    Symtab::free_symbol_table();
    free_tags();
    pack_names();
    check_symbols();
    pack_symbols();
...
bucket* Reader::get_literal(void)
{
...
bucket* Reader::get_name(void)
{
    ...
    return (Symtab::lookup(cache));
}


int Reader::get_number(void)
{
    register int c;
    register int n;
...
void Reader::declare_types(void)
{
    register int c;
    register bucket *bp;
    char *tag;
...
void Reader::declare_start(void)
{
    register int c;
    register bucket *bp;
...
void Reader::read_declarations(void)
{
    register int c, k;
...
void Reader::initialize_grammar(void)
{
    nitems = 4;
    maxitems = 300;
...
void Reader::advance_to_start(void)
{
    register int c;
    register bucket *bp;
    char *s_cptr;
...
void Reader::insert_empty_rule(void)
{
    register bucket *bp, **bpp;

    assert(cache);
    sprintf(cache, "$$%d", ++gensym);
    bp = Symtab::make_bucket(cache);
    Symtab::last_symbol->next = bp;
    Symtab::last_symbol = bp;
...
void Reader::pack_names(void)
{
    register bucket *bp;
    register char *p, *s, *t;

    name_pool_size = 13;  /* 13 == sizeof("$end") + sizeof("$accept") */
    for (bp = Symtab::first_symbol; bp; bp = bp->next)
...
void Reader::check_symbols(void)
{
    if (goal->classic == UNKNOWN)
    {
	undefined_goal(goal->name);
    }

    for (bucket* bp = Symtab::first_symbol; bp; bp = bp->next)
    {
	if (bp->classic == UNKNOWN)
	{
...
void Reader::print_grammar(void)
{
    register int i, j, k;
    int spacing = 0;
